  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold text-gray-900">Gestión de Personal</h2>
          <p className="text-gray-600 text-sm mt-1">Administra los miembros de tu equipo</p>
        </div>
        ...
        <Card className="border-2 border-dashed border-gray-200 bg-gray-50/50">
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-gray-600 text-center mb-6 max-w-md text-sm">
              Comienza agregando miembros a tu equipo para gestionar las citas de manera más eficiente
            </p>
            <Button onClick={() => setShowForm(true)} className="bg-blue-600 hover:bg-blue-700 text-white">
              <Plus className="h-4 w-4 mr-2" />
...
const StaffCard = ({ 
  ...
  return (
    <Card className="hover:shadow-md transition-shadow duration-200 border border-gray-200 bg-white">
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <CardTitle className="text-base font-medium text-gray-900 truncate">{staff.full_name}</CardTitle>
              {staff.email && (
          ...
          <Badge 
            variant={staff.is_active ? "secondary" : "outline"}
            className={staff.is_active ? "bg-green-50 text-green-700 border-green-200" : "bg-gray-50 text-gray-600 border-gray-200"}
          >
      ...
      <CardContent className="pt-0 space-y-3">
        {staffServices.length > 0 && (
          <div>
            <h4 className="text-xs font-medium text-gray-700 mb-2">Servicios:</h4>
            <div className="flex flex-wrap gap-1">
              {staffServices.slice(0, 3).map((service: any) => (
                <Badge 
                  key={service.id} 
                  variant="outline" 
